...
// Helper macro to log all presses of keys.  It's very verbose, but helpful 
// to track down issues
// #define DEBUG_KEY_PRESSES

class ModState {
	// Track the state of a modifier (Shift, Alt, Control) key, so we can
	// type something like "aaAAAaa" by only pressing and releasing shift once.
public:
	SHORT VK; // The VK of this modifier
	bool WantDown;	// Do we want this modifier to be down?
	bool StayDown;	// Do we want to force this modifier down?
	bool IsDown;	// Is this modifier down right now?
	string Name;	// The name to show the user of this modifier
	...
	void Hold() {
		// We want to force this key down
	...
	void Release() {
		// We want to stop forcing this key down
		this->StayDown = false;
	}

	void Press(bool down) {
		// Press or release this key
		if (this->WantDown) {
			// We were told to put this key down, so down it goes
			this->WantDown = false;
			down = true;
		}
		if (this->StayDown) {
			// We were told to keep this key down, so it stays down
			down = true;
		}
		if (down != this->IsDown) {
			// If the state changed, then press or release the key
			this->IsDown = down;
...
extern "C" void handle_print_impl(int fd, void* buf, int len) {
	// Handle calls to write to streams like "print".  fd is 1 for stdout, 2 for stderr

	// A len of 0 is a flush call, technically.  Perhaps we should output a string
	// if we have one, if this happens, but for now, just ignore it.
	...
	if (len > 0) {
		...
		for (int i = 0; i < len; i++) {
			...
			case '\r':
				...
				break;
			case '\n':
				// We hit a newline, go ahead and output the string
				if (TestLogEnabled()) {
				...
				*dest = "";
				break;
			default:
				// Anything else just gets built up.
				*dest += ((char*)buf)[i];
				break;
			}
		}
...
void press_key(char key, SHORT vk, ModState& shiftState, ModState& altState, ModState& ctrlState) {
	// Helper to press a single key.  "key" can be a character, which we'll decode and 
	// figure out the VK to press, or "key" can be 0 and vk can be the VK we should rpess.
	...
	if (key != 0) {
		switch (key) {
		case '\n':
			// LF gets turned into the return key
			vk = VK_RETURN;
			break;
		case '\r':
			// Ignore CRs
			...
			break;
		case '\t':
			// Tab characters get turned into tab keys
			vk = VK_TAB;
			break;
		case '\b':
			// Backspace characters get turned into backspace keys
			vk = VK_BACK;
			break;
		default:
			// Everything else, just ask Windows how to press this key
			vk = VkKeyScanA(key);
			break;
		}
	}

	// Get the scan code for the given key
	...
	// And tell our mod helpers to press or release as needed
...
#else
		if (TestLogEnabled()) {
			// This rather length section lets us log the key presses for testing
			// This code is autogenerated by create_key_constants.py
			...
			// In normal mode, just press and release the key
			keybd_event((BYTE)vk, scanCode, extended, 0);
			keybd_event((BYTE)vk, scanCode, extended | KEYEVENTF_KEYUP, 0);
			// Note that we don't release the modifier, since it might be used
			// for the next key.  This requires one lass pass with all of the 
			// modifiers to make sure they're released after any key presses.
		}
#endif
	}
}
...
extern "C" void keys_log_invoke(const char* msg) {
	// Just log the message.  Think of this as "print", though
	// in the long term, we might want to make this do something 
	// more interesting
	if (TestLogEnabled()) {
		stringstream ss;
		ss << "keys.log(\"" << msg << "\")";
...
extern "C" void keys_press_invoke(const char* msg) {
	// Press a sequence of keys.  This will interpret some keys
	// Notable it uses "^%+" for modifier keys, "{}" for special
	// keys, and "()" to group keys together.
	...
	// Prase out the the string we were given, looking for special keys
		...
		if (in_curly) {
			if (curly.size() > 0 and *cur == '}') {
				...
				if (curly.size() == 1) {
					...
					// All of the special keys are autogenerated
...
extern "C" void keys_press_raw_invoke(const char* msg) {
	// Handle pressing of keys, without parsing for special keys
...
void AddDesc(stringstream& desc, string val) {
	// Helper to build up a key description.  Just add "+" between parts
	if (desc.tellp() > 0) {
		desc << "+";
	}
	desc << val;
...
extern "C" void key_press_store_fun(int vk, char* name, void* fun) {
	// This is called at the end of a small chain in response to the keys.key decorator
	KeyData key;
	key.ID = (int)GetKeys().size();
	if (vk != 0) {
		// We were told this key is for a given hotkey, so go ahead and register that key
	...
	stringstream ss;
	if (vk != 0) {
		// And get the key's description that we registered to show the user
		...
		// Or, if this key is for a named macro, show that name
		ss << "Registered '" << key.Name << "'";
	}

	// Store the information for this function in our list
...
extern "C" char* clip_copy_invoke() {
	// Tee macro wants the contents of the clipboard, so get it, and return to the user
...
extern "C" void clip_paste_invoke(const char* data) {
	// The macro wants to set the clipboard contents, so do that work
...
extern "C" void mouse_position_invoke(int* x, int* y) {
	// Get the current mouse position and return it
	if (TestLogEnabled()) {
...
void mouse_helper(DWORD event, int vk, int check) {
	// Simple helper to press and release a mouse button

	if (!TestLogEnabled()) {
		mouse_event(event, 0, 0, 0, NULL);
		for (int bail = 0; bail < 10; bail++) {
			// Make sure the mouse event happened, might need to delay a bit
			// while Windows catches up
...
extern "C" void mouse_click_invoke(int left_down, int left_up, int right_down, int right_up) {
	// One single helper method to do all of the mouse work
	// Each of the args is really a bool if we should press and/or release each button in turn
	if (TestLogEnabled()) {
		stringstream ss;
...
string GetWindowText(HWND hwnd) {
	// Wrapper for Win32 GetWindowText, handling the string alloc
...
string GetClassName(HWND hwnd) {
	// Wrapper for Win32 GetClassName, handling the string alloc
	int len = 250;
...
string HandleToStr(HWND hwnd) {
	// Turn a Win32 HWND to a string, to make passing it around Python easier
	stringstream ss;
...
HWND StrToHandle(string hwnd) {
	// Turn a pythonized Win32 handle back to a real handle
...
extern "C" void* windows_get_active_impl() {
	// Get the information for the active window
...
BOOL CALLBACK EnumWindowsProc(_In_ HWND hwnd, _In_ LPARAM lParam) {
	// Helper to handle the call to EnumWindows during a list_all call

	// We only care about visible windows
	if (IsWindowVisible(hwnd)) {
		// We only care about non-child windows
		LONG style = GetWindowLong(hwnd, GWL_STYLE);
		if ((style & WS_CHILD) == 0) {
			// Get the title and class name
			string title = GetWindowText(hwnd);
			string classname = GetClassName(hwnd);
...
extern "C" void windows_list_all_impl(void*** list, int* count) {
	// Helper to get the list of all visible windows
...
extern "C" void windows_set_active_impl(const char* handle) {
	// Helper to set the foreground window to a given window
...
extern "C" void windows_launch_impl(const char* command_line) {
	...
	// Pass the string as is off to CreateProcess to do the work of cracking it
	STARTUPINFOA si = { 0 };
